...
/** This function will read in a wordlist, as per the format in the
 * CS 2150 lab 6 document, into a hash table.
 *
 * @return hashtable containing words from the wordlist
 *
 * @param filename The file name to read in -- it's assumed to be in
 *                 the file format described in the lab document.
 */
HashTable* readInDict (string filename) {
    //variable to hold the size of the wordlist
    int count=0;
    ...
    //Initialize hashtable with the next prime number
    HashTable *myTable = new HashTable(getNextPrime(count));

    ifstream file2(filename.c_str());

    //Inser each word into the table
    while(getline(file2, line)) {
        for (int i=3; i<=line.length(); i++) {
            myTable->insert(line.substr(0, i-1)+"0");
        }
        myTable->insert(line);
    }
