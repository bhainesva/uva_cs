...
\begin{flalign*}
    ...
    &&1 &< 0 &&\text{by (M4)}
\end{flalign*}
This is a contradiction with (O1) because the fact was stated in class that $1>0$, so $x^{-1} > 0$.
...
Now let $x -\epsilon < y + \epsilon$ for all $\epsilon > 0$. Assume that $x > y$. Then $x-y > 0$ so I can set $\epsilon_0 = \frac{x-y}{3}$ and $\epsilon_0$ will be positive (this follows from problem 2.c). Then plugging in I get $x - \frac{x-y}{3} < y + \frac{x-y}{3}$. $\epsilon_0 > 0$ so by (O5)
\begin{flalign*}
...
\part
Let $n$ be the largest natural number such that $\frac{1}{n} < y-x$. Let $k$ be the largest natural number such that $\frac{k}{n}\le x$. Then by the selection of $k$, $\frac{k+1}{n} > x$. 

Now assume that $y \le \frac{k+1}{n}$. Then I have that $\frac{k +1}{n} \ge y$ and 
\begin{flalign*}
    &&\frac{k}{n} &\le x&&\\
    &&\frac{k}{n} + (-x)&\le x + (-x)&&\text{(05)'}\\
    &&\frac{k}{n} + (-x)&\le 0&&\text{(A4)}\\
    &&-(\frac{k}{n}) + \frac{k}{n} + (-x)&\le -(\frac{k}{n}) + 0&&\text{(O5)'}\\
    &&0 + (-x)&\le -(\frac{k}{n}) + 0&&\text{(A4)}\\
    &&-x &\le -(\frac{k}{n})&&\text{(A3)}
\end{flalign*} 
so by (O5)'':
