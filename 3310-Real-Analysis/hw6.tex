...
\part
\begin{enumerate}
    \item $\lim\limits_{x\to a}f(x) = L \iff \lim\limits_{x\to a}|f(x) - L| = 0$
        This follows directly from the arithmetic properties of limits and part a) of problem 4.
        %Be definition $\forall \epsilon > 0$ $\exists \delta > 0$ such that $|f(x) - L| < \epsilon$ for all $x$ such that $|x - a| < \delta$.
    \item $f(x) - L \le g(x) \forall x, \lim\limits_{x\to a} g(x) = 0 \implies \lim\limits_{x\to a} f(x) = L$. 
...
\problem{6}
\part
Let $\epsilon = \sqrt{5}$. We know by definition of the limit that for all $n > 7 + \frac{10}{\epsilon^2} = 9$, $|a_n - 4| < \sqrt{5}$. So $-\sqrt{5} < a_n - 4 < \sqrt{5} \implies 4-\sqrt{5} < a_n < 4 + \sqrt{5}$. $-(4 + \sqrt{5}) < 4 - \sqrt{5}$ so $-(4+\sqrt{5}) < a_n < (4 + \sqrt{5} \implies |a_n| < 4 + \sqrt{5}$. Thus the conditions are satisfied for $N = 9, C = 4 + \sqrt{5}$. 
...
\begin{align}
    |a_{n}^2 - 16| &= |(a_n - 4)(a_n + 4)|\\
                   ...
                   &= \epsilon
\end{align}
So for any $\epsilon > 0$ for all $n \ge M(\epsilon) = \max\left(9, 7 + \frac{10}{(\epsilon/(8 + \sqrt{5}))^2}\right)$, $|a_{n}^2 - 16| < \epsilon$.
\problem{7}
\part
...
\problem{8}
\part
For this problem I will treat $\frac{1}{\infty}$ as 0. 
\begin{enumerate}
    \item WTS: $x = y \iff d(x, y) = 0$\\
        Assume that $x = y$. Then
$$d(x, y) = \left|\frac{1}{x} - \frac{1}{y}\right| = \left|\frac{1}{x} - \frac{1}{x}\right| = 0$$
        Assume that $d(x, y) = 0$. Then 
        \begin{align*}
            \left|\frac{1}{x} - \frac{1}{y}\right| &= 0\\
                                                   &\implies \frac{1}{x} - \frac{1}{y} &= 0\\
                                                   &\implies \frac{1}{x} = \frac{1}{y}\\
                                                   &\implies y = x
        \end{align*}
    \item WTS: $d(x, y) = d(y, x)$ for all $x, y\in X$.\\
        By the property of absolute values:
        $$d(x, y) = \left|\frac{1}{x} - \frac{1}{y}\right| = \left|\frac{1}{y} - \frac{1}{x}\right| = d(y, x)$$
    \item WTS: $d(x, z) \le d(x, y) + d(y, z)$ for all $x, y, z\in X$.\\
        Using the triangle inequality:
        $$d(x, z) = \left|\frac{1}{x} - \frac{1}{z}\right| \le \left|\frac{1}{x} - \frac{1}{y}\right| + \left|\frac{1}{y} - \frac{1}{z}\right| = d(x, y) + d(y, z)$$
\end{enumerate}
Thus $(X, d)$ is a metric space.

\part
First assume that $\lim\limits_{n\to\infty} = L$ in the usual sense. Fix some $\epsilon > 0$. By definition there exists some $N\in \mathbb N$ such that $\forall n \ge N$ $|f_n - L| < \epsilon$. We know that each $f_n$ equals $f(x)$ for some $x\in X$. We need to find a $\delta$ such that for all $x\in B_{\delta}^{\circ} = \Set{x\in \mathbb N : d(x, \infty) < \delta}$, $|f(x) - L| < \epsilon$. By definition $d(x, \infty) = |1/x|$ ($x\in \mathbb N$) so we want to find a $\delta$ such that $1/x < \delta \implies x \ge N$. Let $\delta = \frac{1}{N}$. Then for all $x$
\begin{align*}
    d(x, \infty) = \frac{1}{x} < \delta &\implies \frac{1}{x} < \frac{1}{N}\\
                                        &\implies x > N \\
                                        &\implies |f_x - L| < \epsilon\\
                                        &\implies |f(x) - L| < \epsilon
\end{align*}
So $\lim\limits_{n\to\infty}f(n) = L$ as a limit in the metric space.

Now assume $\lim\limits_{n\to\infty}f(n) = L$ as a limit in the metric space. Fix some $\epsilon > 0$. By definition there exists a $\delta$ such that for alll $x\in B_{\delta}^{\circ}$, $|f(x) - L| < \epsilon$. Let $N = \lceil \frac{1}{\delta}\rceil + 1$. Then for all $n \ge N$.
\begin{align*}
    n \ge N &\implies n \ge \lceil\frac{1}{\delta}\rceil + 1\\
            &\implies n > \frac{1}{\delta}\\
            &\implies d(n, \infty) = \frac{1}{n} < \delta\\
            &\implies |f(n) - L| < \epsilon\\
            &\implies |f_n - L| < \epsilon
\end{align*}
So $\lim\limits_{n\to\infty}f_n = L$ in the usual sense.
\end{document}
