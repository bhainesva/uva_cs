...
\part
It is clear from the definition of $S$ that it is a subset of $\mathbb R$. In order to show that $S = [a, b]$ is closed we must show that $S\setminus \mathbb R$ is open. $S\setminus \mathbb R = \Set{x\in R : x < a\text{ or }x > b}$. So the complement of $S$ can be written as the union of the two intervals $(-\infty, a)$ and $(b, \infty)$. We want to show that for any number $x$ in either of those intervals, there exists an $\epsilon > 0$ such that $(x-\epsilon, x + \epsilon)$ is a subset of the particular interval. First select an $x$ from $(-\infty, a)$. By definition, $x < a$ so let $\epsilon = \frac{a-x}{2}$. Then $x-\frac{a-x}{2} > -\infty$ and $x + \frac{a-x}{2} < a$. Now select an $x$ from $(b, \infty)$. By definition $x > b$ so let $\epsilon = \frac{x-b}{2}$. Then $x-\frac{x-b}{2} > b$ and $x+\frac{x-b}{2} < \infty.$ Thus $S\setminus \mathbb R$ is an open subset of $\mathbb R$ so $S$ is a closed subset of $\mathbb R$.
...
\solution
\part
WTS: $B_\epsilon(x) = \Set{y\in X : d(x, y) < \epsilon}$ is equal to $(x-\epsilon, x+\epsilon) = \Set{y\in \mathbb R : x-\epsilon < y < x+\epsilon}$. We know $X = \mathbb R$ and $d(x, y) = |x - y|$ so $B_\epsilon(x)$ can be rewritten as:
    $$\Set{y\in \mathbb R : |x-y| < \epsilon}$$
by properties 2 and 6 of absolute values:
\begin{align}
    \begin{split}
        \Set{y\in \mathbb R : |x-y| < \epsilon} &= \Set{y\in \mathbb R : |y-x| < \epsilon}\\
                                                &= \Set{y\in \mathbb R : -\epsilon < y-x < \epsilon}\\
                                                &= \Set{y\in \mathbb R: x - \epsilon < y < x + \epsilon}
    \end{split}
\end{align}
So the sets are equivalent.
\part
In the first space it forms a disk. It selects all points less than a given cartesian distance from a central point. In the second space it forms a filled in square. These are shown in the picture below.
\vspace{20mm}
\problem{8}
Prove that if $S$ is any open ball in $X$ (that is, $S = B_\alpha(y)$ for some $y\in X$ and $\alpha > 0$, then $S$ is an open subset of $X$. 
\solution
$S = B_\alpha(y) = \Set{z\in X : d(y, z) < \alpha}$.
Select an arbitrary $x\in B_\alpha$. We want to show that there exists some $\epsilon > 0$ such that $B_\epsilon(x) = \Set{z\in X : d(x, z) < \epsilon} \subseteq S$.
