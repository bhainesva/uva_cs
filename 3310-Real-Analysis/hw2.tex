...
\begin{enumerate}[(a)]
    ...
    \item Prove that if $S$ is a closed interval $(S = [a,b] = \Set{x\in \mathbb R : a \le x \le b}$ for some $a\leb$), then $S$ is a closed subset of $\mathbb R$. 
\end{enumerate}
\solution
\part
...
\problem{7}
\begin{enumerate}[(a)]
    \item Let $X = \mathbb R$ and $d(x, y) = |x-y|$. Recall that $(X, d)$ is a metric space by HW\#1.6(b). Prove that $B_\epsilon(x) = (x-\epsilon, x+\epsilon)$ for all $x\in X$ and $\epsilon > 0$ (thus, an open ball of radius $\epsilons$ centered at $x$ in this case is simply the open interval of length $2\epsilon$ centered at $x$). \textbf{Hint:} The result follows directly from basic properties of absolute values.
